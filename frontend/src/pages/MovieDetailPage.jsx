...
function MovieDetailPage() {
  ...
  const fetchDetails = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/movie?${query.toString()}`);
      ...
      if (typeof document !== 'undefined') {
        ...
        document.title = titlePieces.length ? `KAM • ${titlePieces.join(' ')}` : 'KAM • Movie Details';
      }
  ...
  const posterImage = detail?.posterUrl || detail?.posterUrlPlex || detail?.plexPosterUrl || null;
  const backgroundImage = detail?.backgroundUrl || detail?.backgroundUrlPlex || detail?.plexBackgroundUrl || null;
